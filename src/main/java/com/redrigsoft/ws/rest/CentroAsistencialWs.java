	...
	public String sincronizarCentrosAsistencialesPOST(String v_todosCentrosAsistenciales, @HeaderParam("authorization") String v_claveApi){ 
		...
		if(id_Usuario!=0){
			...
			if(partes!=null && partes.length==1) {
			     ...
			     todosCentrosAsistenciales = partes[0];
			     codDispositivo 		   = partes[1];
			     codToken 			       = partes[2];
			     CuentaEssaludDao.actualizarFechaUltimaSync(3, codDispositivo, codToken);
			}
			...
			String arrayCentros =   gson.toJson(L6);//  CentroAsistencialDao.obtenerCentrosAsistencialesEssaludJson(L6); 		
